...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    const symbols = array.filter((sym) => sym === array[i]);
    if (symbols.length === 1) {
      return array[i];
    }
  }
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startSym}${smallNum}, ${bigNum}${endSym}`;
}
...
function isCreditCardNumber(ccn) {
  const cardArr = ccn.toString().split('').map(Number);
...
function getDigitalRoot(num) {
  ...
  return counter >= 10 ? Math.floor(counter / 10) + (counter % 10) : counter;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBrackets.indexOf(str[i]) >= 0) {
      stack.push(str[i]);
    } else if (closeBrackets.indexOf(str[i]) !== openBrackets.indexOf(stack.pop())) {
      return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < splitPathes[0].length; i += 1) {
    const partial = splitPathes[0][i];
    if (splitPathes.every((path) => path[i] === partial)) {
      res.push(`${partial}/`);
    }
  }
